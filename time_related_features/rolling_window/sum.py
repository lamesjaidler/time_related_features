...
class SumValueChargebacks(_Base):
    """
    Calculates the sum of the `value_col` for chargeback transactions, for
    the period given, for a given container key.
    ...
    **kwargs: dict
    ...
    def transform(self,
        ...
        """
        X = X.assign(value_col_cb_only=X[self.cb_col] * X[self.value_col])
        rw = self._return_rw(X=X, period=period)
        agg = rw['value_col_cb_only'].sum()
        agg = _update_agg_name(
            agg=agg, output_name=output_name,
            default_name=f'{self.container_key}.sum_chargeback_{self.value_col}_per_{self.container_key}_{period}{self.period_type}'
        )
        return [agg]


class SumValue(_Base):
    """
    Calculates the sum of the `value_col` for the period given.    
    ...
    **kwargs: dict
    ...
    def transform(self,
        ...
        """
        rw = self._return_rw(X=X, period=period)
        agg = rw[self.value_col].sum()
        agg = _update_agg_name(
            agg=agg, output_name=output_name,
            default_name=f'{self.container_key}.sum_{self.value_col}_per_{self.container_key}_{period}{self.period_type}'
        )
        return [agg]
