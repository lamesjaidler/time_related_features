from time_related_features.time_deltas.time_since_last_seen import TimeSinceLastSeen
import pandas as pd
import numpy as np
...
def test_time_since_last_seen():
    expected_result = pd.DataFrame(
        np.array([
            ...
            [5.0, pd.Timedelta('1 days 01:00:00')],
            [6.0, pd.Timedelta('1 days 01:00:00')],
            ...
            [7.0, pd.Timedelta('4 days 04:00:00')],
            [9.0, pd.Timedelta('1 days 02:00:00')]],
            dtype=object),
        columns=['eid', 'email.time_since_txn_last_seen_by_email']
    )
    expected_result.set_index('eid', inplace=True)
    expected_result = expected_result.squeeze()
    X = _create_data()
    agg_class = TimeSinceLastSeen()
