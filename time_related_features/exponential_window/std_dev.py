...
class ExpWeightedStdDev(_Base):
    """
    Calculates the historic exponentially weighted moving standard deviation of 
    ...
    **kwargs: dict
    ...
    def transform(self,
        ...
        agg = ewm[self.value_col].std()
        agg = _update_agg_name(
            agg=agg, output_name=output_name,
            default_name=f'{self.container_key}.ew_std_dev_{self.value_col}_per_{self.container_key}'
        )
        return agg
